...
target_file = 'Interview_Partner_Tracking_Team_Final.xlsx'
...
for st in STATUS_LIST:
    ...
    # Using Column F (6) for Status. Range F5:F500 to cover everything
...
for name, data in sheets.items():
    ...
    for r_idx, row in enumerate(data.itertuples(index=False), 5):
        for c_idx, val in enumerate(row, 1):
            ...
            # 1. Date Format (Col J / 10)
            if cols[c_idx-1] == 'Datum':
                cell.number_format = 'DD.MM.YYYY'
                cell.alignment = center_align
            
            # 2. Time Format (Col K / 11)
            elif cols[c_idx-1] == 'Uhrzeit':
                cell.number_format = 'HH:MM'
                cell.alignment = center_align
                
            # 3. Duration Format (Col L / 12)
            elif cols[c_idx-1] == 'Dauer':
    ...
    # 4. Themes/Topics (Col O / 15)
    try:
        topic_idx = cols.index('Themen') + 1
        topic_letter = get_column_letter(topic_idx)
        dv_topic = DataValidation(type="list", formula1=f'"{topic_string}"', allow_blank=True)
        ws.add_data_validation(dv_topic)
        dv_topic.add(f'{topic_letter}5:{topic_letter}{last_row}')
    except: pass
...
wb.save(full_path)
print(f'Final Master File Saved: {full_path}')
