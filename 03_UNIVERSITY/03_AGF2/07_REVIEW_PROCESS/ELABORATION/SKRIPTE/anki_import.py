...
# Finde den Obsidian-Vault-Pfad (Annahme: Das Skript ist tief im Vault)
# C:\Users\andre\Meine Ablage\Obsidian\03_UNIVERSITY\03_AGF2\07_REVIEW_PROCESS\ELABORATION\SKRIPTE\anki_import.py
# Wir müssen 5 Ebenen hoch, um zum 'Obsidian' Ordner zu gelangen.
try:
    OBSIDIAN_VAULT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", ".."))
    KARTEIKARTEN_ROOT_DIR = os.path.join(OBSIDIAN_VAULT_ROOT, "19_FLASHCARDS")
except Exception:
...
def parse_markdown_cards(file_path):
    ...
    cards = []
    # Splitten der Datei in einzelne Karten-Abschnitte
    parts = re.split(r'\n---\n', content)
    
    for part in parts:
        part = part.strip()
        # Suche nach Frage (## Frage X: ...) oder nur (## ...)
        question_match = re.search(r'^(?:##|#)\s*(?:Frage\s*\d*:)?\s*(.+)', part)
